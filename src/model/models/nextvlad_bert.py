...
class NextVladBERT(BaseModel):
    def __init__(self, model_config):
        ...
        for modal in (self.modal_name_list + ['fusion']):
            ...
            self.fusion_head_dict[modal] = fusion_head.get_instance(model_config['fusion_head_type'],
                                                                    fusion_head_params)
            self.classifier_dict[modal] = classcify_head.get_instance(model_config['tagging_classifier_type'],
                                                                      model_config['tagging_classifier_params'])
            if modal == 'video':
                self.head_dict[modal] = video_head.get_instance(model_config['video_head_type'],
                                                                model_config['video_head_params'])
            elif modal == 'audio':
                self.head_dict[modal] = video_head.get_instance(model_config['audio_head_type'],
                                                                model_config['audio_head_params'])
            elif modal == 'text':
                self.head_dict[modal] = text_head.get_instance(model_config['text_head_type'],
                                                               model_config['text_head_params'])
            elif modal == 'image':
                self.head_dict[modal] = image_head.get_instance(model_config['image_head_type'],
                                                                model_config['image_head_params'])
            elif modal == 'fusion':
                pass
            else:
    ...
    def __call__(self, inputs_dict, is_training=False, train_batch_size=1):
        ...
        for modal_name in self.modal_name_list:
            ...
            if is_training and self.use_modal_drop:
                inputs_dict[modal_name], prob_dict[modal_name + '_loss_weight'] = self._modal_drop(
                    inputs_dict[modal_name], self.modal_drop_rate, drop_shape)

            with tf.variable_scope(modal_name):
                if modal_name in ['video', 'audio']:
                    embedding = self.head_dict[modal_name](inputs_dict[modal_name], is_training=is_training,
                                                           mask=mask,
                                                           num_frames=inputs_dict[modal_name + '_frames_num'])
                else:
                    embedding = self.head_dict[modal_name](inputs_dict[modal_name], is_training=is_training)

            with tf.variable_scope("tag_classifier/" + modal_name[0]):
                if self.with_embedding_bn:
                    embedding = slim.batch_norm(embedding, center=True, scale=True, is_training=is_training,
                                                scope=modal_name[0] + "_feat_bn")
                encode_emb = self.fusion_head_dict[modal_name]([embedding], is_training=is_training,
                                                               in_matrix=inputs_dict['in_matrix'],
                                                               out_matrix=inputs_dict['in_matrix'],
                                                               label_feature=inputs_dict['label_feature'])
        ...
        with tf.variable_scope("tag_classifier/fusion"):
            fusion_embedding = self.fusion_head_dict['fusion'](embedding_list, is_training=is_training,
                                                               ...
                                                               out_matrix=inputs_dict['in_matrix'],
                                                               label_feature=inputs_dict['label_feature'])
            probs = self.classifier_dict['fusion'](fusion_embedding, is_training=is_training)
        ...
        for key, loss_fn in label_loss_fn_dict.items():
            if key == 'tagging':
                ...
                for modal in self.modal_name_list + ['fusion']:
                    ...
                    params = results["tagging_output_" + modal]
                    params['loss_weight'] = loss_weight
                    params['labels'] = labels
                    loss_dict["tagging_loss_" + modal] = loss_fn.calculate_loss(**params)
            else:
                raise NotImplementedError
        return loss_dict
