...
def FormatEvalInfo(summary_writer,
    ...
    info = "epoch/eval number {} | MAP: {:.3f} | GAP: {:.3f} | p@0.1: {:.3f} | p@0.5:{:.3f} | r@0.1:{:.3f} | r@0.5: {:.3f} | Avg_Loss: {:3f}".format(
        epoch_id, mean_ap, gap, precision_at_1, precision_at_5, recall_at_1, recall_at_5, avg_loss)
    return info
...
def later_fusion(preds_dict, modal_name_list, method="avg"):
    return preds_dict['fusion']
    preds = []
    for modal_name in modal_name_list:
        if modal_name not in preds_dict:
            raise RuntimeError("modal name :{} not in preds_dict:{}".format(modal_name, preds_dict))
        preds.append(preds_dict[modal_name])

    is_tensor = False
    if tf.is_tensor(preds[0]):
        is_tensor = True
...
def run_test_later_fusion():
    preds_dict = {
        'video': tf.constant([[0.9, 0.1, 0.91, 0.2], [0.9, 0.1, 0.91, 0.2]]),
        'audio': tf.constant([[0.8, 0.3, 0.89, 0.01], [0.8, 0.3, 0.89, 0.01]]),
        'fusion': tf.constant([[0.92, 0.001, 0.98, 0.03], [0.8, 0.3, 0.89, 0.01]])
    }
    preds_dict = {
        'video': np.array([[0.9, 0.1, 0.91, 0.2], [0.9, 0.1, 0.91, 0.2]]),
        'audio': np.array([[0.8, 0.3, 0.89, 0.01], [0.8, 0.3, 0.89, 0.01]]),
        'fusion': np.array([[0.92, 0.001, 0.98, 0.03], [0.8, 0.3, 0.89, 0.01]])
    }
    preds = later_fusion(preds_dict, modal_name_list=['video', 'fusion', 'audio'])
    print(preds)
    print(preds.shape)


if __name__ == '__main__':
    run_test_later_fusion()
