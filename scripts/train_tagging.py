...
class TaggingTrainer(Trainer):
    ...
    def load_pretrained_model(self):
        ...
        image_pretrained_model = self.optimizer_config.pretrained_model['image_pretrained_model']
        assignment_map, _ = train_util.get_assignment_map_from_checkpoint(tf.global_variables(),
                                                                          image_pretrained_model,
                                                                          var_prefix='tower/image/',
                                                                          show=True)
        tf.train.init_from_checkpoint(image_pretrained_model, assignment_map)
        print("load image_pretrained_model: {}".format(image_pretrained_model))
    ...
    def train_metric_log(self, train_fetch_dict_val, examples_per_second):
        ...
        logging.info("training step {} |{} | Examples/sec: {:.2f}".format(global_step_val, train_losses_info,
                                                                          examples_per_second))
        logging.info(
            "GAP: {:.2f} | precision@0.1: {:.2f} | precision@0.5: {:.2f} |recall@0.1: {:.2f} | recall@0.5: {:.2f}".format(
                gap,
        ...
        self.sv.summary_writer.add_summary(train_util.MakeSummary("TrainMetric/precision@0.1", precision_at_1),
                                           global_step_val)
        self.sv.summary_writer.add_summary(train_util.MakeSummary("TrainMetric/precision@0.5", precision_at_5),
                                           global_step_val)
        self.sv.summary_writer.add_summary(train_util.MakeSummary("TrainMetric/recall@0.1", recall_at_1),
                                           global_step_val)
        self.sv.summary_writer.add_summary(train_util.MakeSummary("TrainMetric/recall@0.5", recall_at_5),
                                           global_step_val)
    ...
    def eval(self, sess, global_step_val, data_generater, data_source_name):
        ...
        for sample in data_generater:
            ...
            for index, modal_name in enumerate(self.modal_name_list + ['fusion']):
                ...
                preds_dict[modal_name] = pred
                val_label = sample['tagging']

                # 对各个模态的预测结果进行融合
                if modal_name == 'fusion':
                    pred = train_util.later_fusion(preds_dict, modal_name_list=['video', 'fusion'])

                gap = train_util.calculate_gap(pred, val_label)
                iteration_info_dict = self.evl_metrics[index].accumulate(pred, val_label,
                                                                         fetch_dict_eval['tagging_loss_'+modal_name])
        ...
        for index, modal_name in enumerate(self.modal_name_list + ['fusion']):
            ...
            epochinfo = train_util.FormatEvalInfo(self.summary_writer, global_step_val, epoch_info_dict,
                                                  prefix="val_" + modal_name)
