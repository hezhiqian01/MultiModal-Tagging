...
def parse_gt_txt(gt_txt, tag_dict):
    gt_dict = {}
    with open(gt_txt, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            gt_dict[vid] = convert_to_hot(labels, np.ones(len(labels)), tag_dict)
    return gt_dict
...
def precision_statistic(preds, labels, tag_dict, threshold=0.9):
    """
    统计该模型，在哪些类别上，准确率比较低
    :param preds:
    :param labels:
    ...
    for tag in tag_dict:
        ...
        accuracy = f1_score(y_true, y_pred, average='binary')

        # print("label:{} precision:{}, recall:{}, accuracy:{}".format(tag, precision, recall, accuracy))
        result['label'].append(tag)
        result['precision'].append(precision)
        result['recall'].append(recall)
        result['accuracy'].append(accuracy)
    result = pd.DataFrame().from_dict(result)
    result = result.sort_values("accuracy")
    print(result)
    print("该模型预测accuracy 最小的10个有：{}".format(result['label'].tolist()[:10]))
    print("该模型预测accuracy 最大的10个有：{}".format(result['label'].tolist()[-10:]))

    #     result = result.sort_values("precision")
    #     print(result)
    #     print("该模型预测precision ==1 的标签为：{}".format(result[result.precision == 1]['label'].tolist()))
...
if __name__ == "__main__":
    ...
    #     print(pred_dict)
    gt_dict = parse_gt_txt(args.gt_json, tag_dict)
    ...
    print(preds.shape)
    print(labels.shape)
    precision_statistic(preds, labels, tag_dict, 0.1)

    print("The GAP result is {:.6f}".format(gap))
