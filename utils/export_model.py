...
from utils.train_util import later_fusion

_TOP_PREDICTIONS_IN_OUTPUT = 50
...
class ModelExporter(object):
    ...
    def build_inputs_and_outputs(self):
        ...
        with tf.variable_scope("tower"):
            ...
            preds = {}
            for key in result:
                if "tagging_output" not in key:
                    continue
                modal_name = key.split('_')[-1]
                preds[modal_name] = result[key]['predictions']

            predictions = later_fusion(preds, modal_name_list=['video', 'fusion'])
