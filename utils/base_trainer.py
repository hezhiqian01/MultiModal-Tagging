...
def lr_warm_up_and_decay(lr_base, global_step, lr_type, lr_params, warmup=False, warmup_steps=500):
    warmup_lr = (lr_base * tf.cast(global_step, tf.float32) / tf.cast(warmup_steps, tf.float32))
    lr_params = lr_params[lr_type]
    if lr_type == 'exp':
        decay = tf.train.exponential_decay(lr_base,
                                           global_step,
                                           # max(1, max_step_num // 3),
                                           # decay 3 times
                                           # lr_decay,
                                           # staircase=True
                                           **lr_params
                                           )
    elif lr_type == 'cosine_restart':
        decay = tf.train.cosine_decay_restarts(lr_base,
                                               global_step,  # 当前全局iter迭代次数，传入一个递增的variable
                                               # first_decay_steps=100,  # 第一次衰减结束发生在哪一步，设一个epoch含有100个iter，可以设为2*100
    ...
    if warmup:
        learning_rate = tf.cond(global_step < warmup_steps, lambda: warmup_lr, lambda: decay)
    else:
        learning_rate = decay
...
class Trainer(object):
    ...
    def build_train_graph(self):
        ...
        optimizer_dict = {}
        if self.optimizer_config.optimizer == 'AdamWOptimizer':
            optimizer_class = AdamWOptimizer
        else:
        ...
        for k, v in learning_rate_dict.items():
            if k not in self.modal_name_list + ['classifier']:  # 支持不同模块的学习率独立设置
                ...
                continue
            learning_rate_dict[k] = lr_warm_up_and_decay(v,
                                                         self.global_step,
                                                         self.optimizer_config.lr_type,
                                                         self.optimizer_config.lr_params,
                                                         self.optimizer_config.warmup,
        ...
        with tf.name_scope("train_input"):
            raw_inputs_dict = {data_name: self.reader.data_op_list[i] for i, data_name in
                               enumerate(self.reader.dname_string_list)}
            tower_inputs = {key: tf.split(value, num_towers) for key, value in raw_inputs_dict.items()}
            tower_inputs = [{key: value[i] for key, value in tower_inputs.items()} for i in
                            range(num_towers)]  # 放置到不同GPU
        tower_gradients = []
            ...
            with tf.device(device_string % i):
                with (tf.variable_scope(("tower"), reuse=True if i > 0 else None)):
                    with (slim.arg_scope([slim.model_variable, slim.variable],
                        ...
                        result_dict = self.model(tower_inputs[i], train_batch_size=self.reader.batch_size // num_towers,
                                                 is_training=True)

                        # 遍历所有分类任务输出
                        for task_name in self.reader.label_num_dict:
                            if task_name in tower_predictions_dict:
                                tower_predictions_dict[task_name].append(
                                    result_dict[task_name + '_output_fusion']["predictions"])
                            else:
                                tower_predictions_dict[task_name] = [
                                    result_dict[task_name + '_output_fusion']["predictions"]]
                        ...
                        for task_name, task_loss_type in self.optimizer_config.loss_type_dict.items():
                            loss_fn = loss_lib.get_instance(task_loss_type,
                                                            paramters_dict=self.optimizer_config.loss_params[task_loss_type])
                        ...
                        gradients = optimizer_dict['classifier'].compute_gradients(total_loss,
        ...
        if self.optimizer_config.clip_gradient_norm > 0:
            with tf.name_scope('clip_grads'):
                merged_gradients = train_util.clip_gradient_norms(merged_gradients,
                                                                  self.optimizer_config.clip_gradient_norm)
        ...
        var_dict_logging_info = '|'.join(
            ["{} vars size: {}".format(name, len(value)) for name, value in vars_dict.items()])
        logging.info('!' * 100)
            ...
            if len(var) > 0:
                if key != 'classifier':
                    ...
                    train_op_list.append(
                        optimizer_dict[key].apply_gradients(vars_dict[key], global_step=self.global_step))
    ...
    def build_eval_graph(self):
        ...
        with tf.name_scope("eval_input"):
            for input_name, input_shape, input_dtype in zip(input_name_list, inupt_shape_list, input_dtype_list):
                ...
                inputs_dict[input_name] = tf.placeholder(shape=[None] + input_shape, dtype=input_dtype,
                                                         name=input_name)  # add batch size dim
        ...
        with tf.variable_scope("tower", reuse=True):
            ...
            for task_name in self.reader.label_num_dict:  # [tagging, classification]
                for modal_name in ['fusion'] + self.modal_name_list:
                    val_tagging_predictions[task_name + '_output_' + modal_name] = \
                        result[task_name + "_output_" + modal_name]["predictions"]
            loss_fn_dict = {}
            for task_name, task_loss_type in self.optimizer_config.loss_type_dict.items():
                loss_fn = loss_lib.get_instance(task_loss_type,
                                                paramters_dict=self.optimizer_config.loss_params[task_loss_type])
            ...
            try:
                ...
                while not self.sv.should_stop():
                    try:
                        ...
                        if self.is_master and train_fetch_dict_eval['global_step'] % 10 == 0 and \
                                self.optimizer_config.train_dir:
                            ...
                            train_losses_info = "|".join(
                                ["{}: {:.3f}".format(k, v) for k, v in train_losses_dict.items()])
                            logging.info(
                                "training step {} | {} | {:.2f} Examples/sec".format(global_step_val, train_losses_info,
                                                                                     examples_per_second))
    ...
    def export_model(self, global_step_val, session, validation_score_dict):
        ...
        model_dir = "{}/export/step_{}_{:.4f}".format(self.optimizer_config.train_dir, global_step_val,
                                                      validation_score)
        self.model_export_pb.export_model(model_dir=model_dir, global_step_val=global_step_val,
...
def train_main(config_path, TrainerType):
    ...
    # 设置随机种子
    if config['OptimizerConfig']['set_seed']:
        setup_seed(config['OptimizerConfig']['seed'])
