...
#################Inference Utils#################
tokokenizer = tokenization.FullTokenizer(vocab_file='pretrained_models/bert/chinese_L-12_H-768_A-12/vocab.txt')
...
class TaggingModel():
    ...
    def text_preprocess(self, txt_dict, max_len=128):
        ocr_text = txt_dict['video_ocr']
        asr_text = txt_dict['video_asr']
        # 如果ocr没有结果（有些视频没有字幕），使用asr结果，否则使用ocr结果
        txt = ocr_text
        if len(ocr_text) < 10 and len(asr_text) > len(ocr_text):
            txt = asr_text
    ...
    def preprocess(self, feat_dict, max_frame_num=300):
        ...
        for feat_type in feat_dict:
            ...
            elif feat_type in ['label_feature', 'in_matrix', 'out_matrix']:
                feats = feat_dict[feat_type]
            else:
                raise RuntimeError()
    ...
    def load_multimodal_feat(self, test_file, feat_dir):
        ...
        feat_dict['video'] = np.load(os.path.join(feat_dir, 'video_npy', video_id + '.npy'))
        feat_dict['audio'] = np.load(os.path.join(feat_dir, 'audio_npy', video_id + '.npy'))
        feat_dict['image'] = cv2.imread(os.path.join(feat_dir, 'image_jpg', video_id + '.jpg'), 1)
        with open(os.path.join(feat_dir, 'text_txt', video_id + '.txt')) as f:
            feat_dict['text'] = json.load(f)
        feat_dict['label_feature'] = np.load('dataset/label_feature.npy')
        feat_dict['out_matrix'] = np.load('dataset/co_occurrence.npy')
        feat_dict['in_matrix'] = np.load('dataset/co_occurrence.npy').T
    ...
    def inference(self, test_file, load_feat=False, feat_dir=None):
        with self.sess.as_default() as sess:
            if load_feat == False:
                raise NotImplementedError("extract feat not support!")
...
if __name__ == '__main__':
    ...
    gen = tqdm(test_files, total=total)
    for test_file in gen:
        try:
        ...
        if args.output_json is not None:
            ...
            cur_output["result"] = [
                {"labels": labels[:args.top_k], "scores": ["%.2f" % scores[i] for i in range(args.top_k)]}]
            # print(cur_output)
        ...
        if args.output is not None:
            with open(args.output, 'a+') as f:
                ...
                f.write("{}\t{}\n".format(video_id, "\t".join(
                    ["{}##{:.3f}".format(labels[i], scores[i]) for i in range(len(scores))])))
        # gen.set_description("processed {}/{}".format(count, total))
    gen.close()
